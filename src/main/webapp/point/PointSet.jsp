...
<form action="/EEIT187-6/Point/UpdatePointSet" method="get" enctype="multipart/form-data">
<% PointSetBean pointSet=(PointSetBean)request.getAttribute("pointSet");%>
    <div class="container">
        ...
        <div>
            ...
            <article>
                <header>集點換算</header>
                每NT$<input type="text" name="perAmount" class="right number" value="<%=pointSet.getAmountPerPoint() %>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                累積<input type="text" name="pointEarn" class="right number" value="<%=pointSet.getPointsEarned() %>">點<span class="remindPoint"></span><br> 
                1點紅利點數可折抵NT$ 1
            </article>
            <article>
                ...
                </select>，紅利點數累積<input type="text" name="ratio" class="right number" value="<%=pointSet.getPointRatio() %>">倍。<span class="remindRatio"></span>
            </article>
            <article>
                <header>點數到期日</header>
                <% String radioOption = pointSet.getIsExpiry(); %>
                <% System.out.println("");%>
                <label ><input type="radio" name="expiration" value="isExpiry" <%= "isExpiry".equals(radioOption) ? "checked" : "" %>>指定到期日</label>
                <label ><input type="radio" name="expiration" value="noExpiry" <%= "noExpiry".equals(radioOption) ? "checked" : "" %>>永不到期</label>
                <br><br>
                在一年內發送的點數會統一在明年的指定日到期。例如：若設定到期日為6/30，則當年度1/1至12/31發送的點數，會統一在明年的6/30到期。<br>
                <select id="month" name="month" value="<%=pointSet.getExpiryMonth() %>">
                ...
                <select id="day" name="day" value="<%=pointSet.getExpiryDay() %>"></select>日
            </article>
            <article>
                <header>點數規則說明</header>
                寫下你的點數使用說明、集點說明集條款，將於會員中心展示。<br>
                <textarea name="comment" id="comment1" rows="10" cols="80" ><%=pointSet.getSetDescription() %></textarea>
            </article>
            <article>
